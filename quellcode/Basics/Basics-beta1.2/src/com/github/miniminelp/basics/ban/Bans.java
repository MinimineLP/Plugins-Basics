...
public class Bans {
	...
	public static boolean isPlayerBanned(String uuid){
		...
		if(bans.contains("BANS."+uuid.toLowerCase())){
			return checkBan(uuid);
		}
		return false;
	}
	...
	public static long getBannTime(Player player){
		if(bans.contains("BANS."+player.getUniqueId().toString().toLowerCase())){
			String s = bans.getString("BANS."+player.getUniqueId().toString().toLowerCase()+".time");
			if(!s.equalsIgnoreCase("LIFETIME"))return Long.parseLong(s);
			else return -1;
		}
		return 0;
	...
	public static boolean checkBan(Player player){
		if(getUnbanTime(player)<1&&getBanType(player)==1){
			bans.set("BANS.DONT_DELEAT_ME_FROM_BANLIST", "BANNED");
			bans.set("BANS."+player.getUniqueId().toString().toLowerCase(), null);
			save();
			return false;
		}
		return true;
