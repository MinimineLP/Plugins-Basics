...
public class Mutes {
	...
	public static boolean isPlayerMuted(Player player){
		checkMute(player);
		return mutes.contains("MUTES."+player.getName().toLowerCase());
	}
	...
	public static String getReason(Player player){
		if(isPlayerMuted(player)){
			return mutes.getString("MUTES."+player.getName().toLowerCase()+".reason").replace("&", "§");
		}
		return null;
	}
	...
	public static String getMuter(Player player){
		if(isPlayerMuted(player)){
			return mutes.getString("MUTES."+player.getName().toLowerCase()+".muter").replace("&", "§");
		}
		return null;
	}
	...
	public static long getMuteTime(Player player){
		if(mutes.contains("MUTES."+player.getName().toLowerCase())){
			String s = mutes.getString("MUTES."+player.getName().toLowerCase()+".time");
			if(!s.equalsIgnoreCase("LIFETIME"))return Long.parseLong(s);
			else return -1;
		}
		return 0;
	...
	public static boolean checkMute(Player player){
		if(getUnmuteTime(player)<1&&getMuteType(player)==1){
			mutes.set("MUTES.DONT_DELEAT_ME_FROM_MUTELIST", "MUTED");
			mutes.set("MUTES."+player.getName().toLowerCase(), null);
			save();
			return false;
		}
		return true;
	...
	public static String getFormattedMuteTime(Player player){
		return getFormattedMuteTime(player.getName());
	}
